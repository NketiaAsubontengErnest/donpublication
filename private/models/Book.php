...
class Book extends Model{
   ...
   public function get_Supply($data){
      ...
      foreach($data as $key =>$row){
         ...
         $query = "SELECT SUM(`quantsupp`) AS ttSupply, (SELECT SUM(`quantsupp` - `retverquant`) FROM `orders` WHERE `bookid` =:bookid AND `seasonid`=:seasonid AND `ordertype` != '1') AS actualSupply FROM `orders` WHERE `bookid` =:bookid AND `seasonid`=:seasonid AND `ordertype` != '1'";
